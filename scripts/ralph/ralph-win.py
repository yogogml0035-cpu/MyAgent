...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ralph autonomous runner for a task-scoped prd.json on Windows"
    )
    argv = sys.argv[1:]
    if argv and argv[0] in ("claude", "codex"):
        argv = ["--agent", argv[0], *argv[1:]]
    ...
    parser.add_argument(
        "prd_json_arg",
        nargs="?",
        help="Optional positional path to tasks/<requirement-slug>/prd.json.",
    )
    parser.add_argument(
        "--dashboard-port",
        type=int,
    ...
    if args.prd_json_arg:
        if args.prd_json:
            parser.error("Pass prd.json only once, either as --prd-json or as a positional path.")
        args.prd_json = args.prd_json_arg
    if not args.prd_json:
        parser.error("--prd-json must point to tasks/<requirement-slug>/prd.json")
    delattr(args, "prd_json_arg")
