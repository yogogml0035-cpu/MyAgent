---
name: prd
description: "为新功能生成 Product Requirements Document (PRD) 和原始对齐需求归档。在规划功能、启动新项目或需要创建 PRD 时使用。触发词：创建一个prd"
---
...
3. 根据答案生成结构化的 PRD
4. 根据本轮对话中用户与 AI 已对齐的需求内容，生成原始对齐需求归档
5. 从需求描述中提炼唯一的 kebab-case 需求目录名，并保存到 `tasks/<requirement-slug>/prd.md` 和 `tasks/<requirement-slug>/requirements.md`
...
- [ ] 使用 Playwright 在浏览器中验证

要写成：

- [ ] 使用 Playwright 打开 `[url/path]`
- [ ] 执行 `[具体操作]`
- [ ] 页面出现 `[具体文案/组件/状态]`
- [ ] 无控制台错误
...
- [ ] 另一个标准
- [ ] Typecheck/lint 通过
- [ ] **[仅 UI stories]** 使用 Playwright 在浏览器中验证
```

**重要提示：** 
- Acceptance criteria 必须是可验证的，不能模糊。"工作正常"是不好的。"删除前按钮显示确认对话框"是好的。
- **对于任何有 UI 变更的 story：** 始终写明用 Playwright 验证的页面、操作和预期结果，不要只写一句泛泛的“使用 Playwright 在浏览器中验证”。
...
除 `prd.md` 外，必须在同一需求目录下同步创建一份原始对齐需求文档。

### 目的

`requirements.md` 用于保存“用户和 AI 在生成 PRD 前已经对齐的原始业务需求”。它是后续 plan、execute、review 回溯需求来源的依据，不替代 `prd.md`。

### 文件名

- 默认文件名：`requirements.md`
- 如果用户在本次请求中明确指定其他文件名，例如 `requirments.md`，按用户指定的文件名创建
- 不要同时创建多个同义需求归档文件

### 生成规则

- 必须根据当前对话上下文生成，尤其是用户与 AI 已经对齐过的目标、范围、业务场景、验收口径、风险和待确认问题
- 如果对话中已有“需求对齐稿”“目标”“范围”“业务场景”“验收口径”“需要对齐的问题”等结构，优先保留该结构和核心措辞
- 可以轻度整理格式、去掉聊天噪音、合并重复表达，但不要把它改写成实施方案，也不要补造用户没有表达或确认过的需求
- 如果缺少明确对齐稿，则从用户最近的需求描述和澄清回答中提炼一份“原始需求摘要”，并在“待确认问题”中标出不确定点
- 不要保存 system/developer 指令、隐藏推理、模型内部 chain-of-thought、密钥、token、账号凭证或与需求无关的聊天内容
- 如果需求本身讨论 `reasoning_content`、日志、审计等字段，可以作为产品需求记录；但不要把当前 AI 的隐藏推理内容写入归档
...
[尽量保留用户已确认的目标、范围、场景、验收口径和关键约束，可轻度整理]
...
- **格式：** Markdown (`.md`)
- **位置：** `tasks/<requirement-slug>/`
- **文件名：** `prd.md` 和 `requirements.md`（若用户明确指定需求归档文件名，则按用户指定）

### 需求目录命名规则

- `tasks/<requirement-slug>/` 是这个需求的唯一产物根目录。
- `<requirement-slug>` 由 skill 从用户的自然语言需求中自动提炼为 kebab-case。
- 不需要在保存前询问用户目录名。
- 初次创建 PRD 时只创建 `tasks/<requirement-slug>/prd.md` 和 `tasks/<requirement-slug>/requirements.md`（或用户明确指定的需求归档文件名）；不要预先创建 `prd.json`、`plan.md`、`progress.txt`、`screenshots/`、`logs/` 或 `history/`。
- 如果基础 slug 已存在，先尝试从需求描述中补充语义差异，例如 `login-fix-error`、`login-ui-adjustment`。
- 如果无法提炼出语义差异，再使用递增后缀，例如 `login-2`、`login-3`。
- 如果发现与已有需求极其雷同，只提示用户可能相似，不阻止创建。
- 不覆盖、不更新已有需求目录中的 `prd.md` 或需求归档文件。
...
- [ ] Typecheck 通过
- [ ] 使用 Playwright 打开任务列表页，每个 task card 都可直接看到 priority badge
- [ ] 页面无控制台错误
...
- [ ] 选择更改时立即保存
- [ ] Typecheck 通过
- [ ] 使用 Playwright 修改一个任务的 priority，列表中的 badge 立即更新
...
- [ ] 已根据本轮对话生成原始对齐需求归档
- [ ] 已保存到 `tasks/<requirement-slug>/prd.md`
- [ ] 已保存到 `tasks/<requirement-slug>/requirements.md`（或用户明确指定的需求归档文件名）
- [ ] 如发现极其雷同的已有需求，已提示用户但未阻止创建
