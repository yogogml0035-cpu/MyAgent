...
- Progress-log display is a user-facing projection over the append-only event log. The collapsed timeline must keep backend `EventRecord.seq` order, merge provider reasoning deltas only into contiguous `"AI正在思考..."` segments, merge `assistant_answer_delta` only into contiguous `"AI正在生成结果"` segments, and collapse tool-call partial/full/result events with the same `tool_call_id` into one lifecycle row. Initial empty tool-call argument chunks are not a formal `"using_tool"` event; the adapter should wait for meaningful arguments or flush the final call immediately before the tool result. Tool rows keep the first call timestamp and a completion timestamp range. Row diagnostics carry raw merged records for internal/debug use, but per-row expansion and per-row copy use a compact display-only JSON projection: thinking/answer rows expose only final `payload.*_stream.content`, and tool rows expose one `type: "tool_lifecycle"` object with final parsed args and final result content/status rather than partial-call history. The run-level raw JSONL copy remains the authoritative full-fidelity debug export. `values_snapshot` stays out of collapsed rows and may be attached to nearby raw status diagnostics or remain available through raw JSONL copying. Terminal runner events (`task_completed`, `task_failed`, `task_cancelled`, `final_answer`) must include display-safe `payload.live` metadata so the visible timeline ends with clear terminal rows rather than the last internal middleware status.
...
- Test fakes must preserve the production storage contract. `backend/tests/fakes.py::InMemoryTaskStorage` should mirror `PostgresTaskStorage` for run-scoped artifact registration, latest completed run resolution through `/api/tasks/{id}/artifacts/{name}`, fixed-artifact top-level mirroring, legacy fallback, refusal to download unrecorded run files, upload listing that ignores directories, deterministic tool-cache IDs, and fresh tool-cache reads that filter expired entries.
...
- **Fake storage drift**: API, runner, and context tests often use `InMemoryTaskStorage`; keep its artifact, upload, event-cursor, run, and fresh-cache behavior aligned with `PostgresTaskStorage`. A fake that returns arbitrary disk entries, ignores cache expiry, or omits latest-run compatibility URLs can let tests pass while production routes or context assembly fail.
...
- `TodoListMiddleware.after_model` and other `after_model` nodes represent model-output completion/state synchronization, not task preparation. Status mapping must check `after_model` before the generic middleware rule so these rows show `"模型输出已完成"` or another organizing/completion label instead of `"正在准备任务..."`.
...
- Progress log rows in `TaskConversation.tsx` are always expandable `<details>` rows. The collapsed row layout must keep the timestamp in the leftmost column for both status rows and tool rows, the display label in the middle, the per-row JSON copy icon beside it, and the disclosure arrow on the right. Tool rows show one continuous monospace summary line containing the user-facing action label, raw tool name, and bounded parameter summary; do not reintroduce chip-heavy metadata or an expanded "工具 / 参数 / 结果" block. Expanded diagnostics show JSON only; the summary-level copy button copies `displayJson` for that row and must not toggle the disclosure state. Tool row `displayJson` is a single lifecycle object with `type`, `tool_name`, `tool_label`, `tool_call_id`, `created_at`, `completed_at`, `tool_call`, and `tool_result`; valid JSON-string result content is embedded as an object/array, and large result display is truncated with metadata without affecting raw JSONL export. Thinking and answer-generation stream chunks stay hidden behind stable collapsed labels such as `"AI正在思考..."` and `"AI正在生成结果"`; their expanded JSON must show the complete normalized content, but they must not render as AI reply cards.
...
- `buildLiveLogItems()` treats `assistant_thinking_delta` and `assistant_answer_delta` as process-log signals only. Collapsed rows show stable labels (`"AI正在思考..."`, `"AI正在生成结果"`), while expanded JSON aggregates and shows the complete normalized chunk content for that run segment. Pre-tool reasoning chunks must remain visible inside the thinking row JSON before the subsequent `tool_call` row. Repeated `tool_call` deltas with the same tool-call id should merge into one visible tool row so partial argument chunks do not create duplicate or out-of-order rows.
