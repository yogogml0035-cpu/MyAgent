...
- `buildLogClipboardText()` copies raw diagnostic JSONL, one event per line. The default progress card remains a Chinese user-facing timeline; each row expansion must render JSON only (no derived metadata table/labels such as event type, display mode, thinking content, or result status). Merged row diagnostics should still be one valid JSON document, using a `records` array when a collapsed row represents multiple backend events.
...
- Progress log rows in `TaskConversation.tsx` are always expandable `<details>` rows. The collapsed row layout must keep the timestamp in the leftmost column for both status rows and tool rows, the display label in the middle, the per-row JSON copy icon beside it, and the disclosure arrow on the right. Tool rows must show a readable tool-call summary in the row itself: user-facing action label, raw tool name, bounded parameter summary, and result/pending text after expansion. Expanded diagnostics show JSON only; the summary-level copy button copies that row's JSON for debugging and must not toggle the disclosure state. Thinking and answer-generation stream chunks stay hidden behind stable collapsed labels such as `"AI正在思考..."` and `"AI正在生成结果"`; their expanded JSON must show the complete normalized chunk content, but they must not render as AI reply cards.
