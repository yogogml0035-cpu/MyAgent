...
- Task message submission accepts optional selected skill names in `skills`. The backend validates them against the project skill projection before starting a run, then formats visible references such as `[$web-research]` into the effective message passed consistently to storage, automatic title generation, and `runner.start_background()`.
