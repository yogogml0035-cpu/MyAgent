...
- The model provider validates `DEEPSEEK_API_KEY`, maps the safe app-level ID to the provider model `deepseek-v4-flash`, toggles thinking mode via the request `thinking` field, and instantiates `DeepSeekThinkingChatModel` only for the thinking-enabled ID so tool-call history can replay `reasoning_content` without changing the non-thinking provider path.
- DeepSeek thinking tool-call history must be serialized through the local adapter in `app/models/deepseek_thinking.py`: only assistant turns that already contain both `tool_calls` and provider-returned `reasoning_content` may replay `reasoning_content`; non-thinking or missing-reasoning history must strip that field instead of fabricating it from events or older runs.
...
- Conversation context is assembled deterministically on every run by `ConversationContextBuilder`: it reads the current task's Postgres message history, injects a bounded session summary plus the most recent messages, and can add 10-minute tool-cache context before the current HumanMessage. It must not replay run diagnostics such as `assistant_thinking_delta`, raw tool-call arguments, or tool-result event payloads into ordinary chat history. The runner then appends long-term memory recall and resource manifest messages after that deterministic session layer.
...
- Runtime skill sources are mounted into the DeepAgents composite backend as read-only virtual paths before calling `create_deep_agent()`. The default `./skills` resolves relative to `backend/` and is exposed to the model as `/skills/`, so progressive-disclosure reads use paths such as `/skills/web_research/SKILL.md` while task file writes remain confined to the current task workspace.
...
- `GET /api/tasks/{id}/events` is the ordered event polling route. Optional `run_id` is a diagnostics/export filter for one run inside the current task only; invalid run ids must stay a 400, and unknown `after_id` cursors must still fail open instead of silently dropping the stream.
...
- Build agent: callers pass configured local skill directories such as `skills=["./skills"]`; `build_agent()` mounts them read-only and passes DeepAgents virtual sources such as `["/skills/"]`.
...
- **Runtime skill path drift**: Do not pass raw `./skills` straight through to DeepAgents after creating a task-scoped backend. Without the `/skills/` read-only route, DeepAgents resolves skill reads against the task workspace and model calls like `read_file("/skills/web_research/SKILL.md")` fail even though `backend/skills` exists.
...
- `v2_adapter.py` converts provider-exposed reasoning content from `AIMessageChunk.additional_kwargs.reasoning_content` or equivalent reasoning blocks into `thinking_chunk`; `event_converter.py` persists it as `assistant_thinking_delta` with display-safe `live` metadata and keeps the raw reasoning text in `payload.content` rather than the top-level event `message`.
- When a reasoning chunk arrives in the same provider turn as tool-call chunks, `v2_adapter.py` must attach normalized diagnostic linkage (`tool_call_id`, `tool_call_ids`, `tool_calls`) to the thinking event payload. Keep those details in diagnostics only: the collapsed progress label still comes from `payload.live.display_text`, and subgraph thinking must not emit root `message_chunk` deltas.
- `assistant_thinking_delta` must keep both the top-level event `message` and `payload.live.display_text` on the stable `"AI正在思考..."` label. Complete reasoning text belongs in `payload.content`, expanded diagnostics, and raw JSONL only, so legacy consumers that fall back to `message` do not leak provider reasoning into the default UI.
