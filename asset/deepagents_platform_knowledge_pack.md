...
- Automatic history naming is best-effort and must not block the task lifecycle indefinitely. Provider errors or timeout fall back to the local first-message title; model output is still normalized server-side to remove prefixes/quotes/newlines and enforce the 10-character display contract. Do not overwrite an existing custom title when later messages are sent.
