...
- `frontend/tests/workspace/test_workspace_view.test.ts`: user -> execution-log card -> final-answer card -> artifact order, final-answer card wording, activity/reasoning/file-audit/search/orchestration clipboard text, collapsed-row copy behavior, composer-to-message-card alignment CSS, text-only empty-state wordmark CSS, and reference robot-avatar CSS.
