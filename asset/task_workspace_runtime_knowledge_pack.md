...
- Search synthesis output is represented internally as `answer`, up to five bounded `sources` with `title`, `url`, and short `snippet`, `used_model`, optional `warning_code`, and safe `event_payload`. Completion event payloads must not include raw Tavily JSON or raw provider content. Displayable assistant answer streaming uses separate bounded `assistant_answer_delta` events with `schema_version: 1`, increasing `stream_index`, cumulative sanitized `content`, and answer-status live metadata. Successful model synthesis must not append a hard-coded `参考来源` block; if the model includes references in its own Markdown answer, render that answer as-is.
- Safe external search source URLs remain valid clickable `http` or `https` URLs through source summarization, synthesis prompts, event payloads, and model-authored Markdown answers. Source URL sanitization must drop unsafe/local URLs such as localhost, loopback/private/link-local/reserved IPs, non-web schemes, userinfo-bearing URLs, or URLs with sensitive query keys; it must not rewrite valid external source URLs into local relative paths or `<absolute-path>` placeholders.
- Model-authored Markdown links are preserved only when their inline or reference-style destination is a safe public `http` or `https` URL. Unsafe, local, non-web, relative, path-like, or redacted destinations must be unwrapped to non-clickable text before frontend rendering.
- Search/weather events are ordered as `search_tool_call`, `search_tool_result`, answer-generation status, optional `assistant_answer_delta` stream events, `search_synthesis_completed`, then terminal `search_completed` in the same completion transition when sources are available. If `TAVILY_API_KEY` is missing, the final answer is a Chinese warning that search is not enabled. If provider synthesis is unavailable, the final answer is a Chinese warning plus a bounded deterministic source summary.
...
- Warning/error event payloads that record caught exceptions must use a safe exception summary with stable `error_type` plus sanitized bounded `error`. They must not persist raw `str(exc)`, authorization headers, access tokens, provider keys, known canaries, uploaded body text, or private absolute paths.
...
- Robot-authored assistant messages, execution-log rows, and artifact rows use the reference TenderWord row-level robot avatar treatment: a 32px square blue-100 marker with a blue-600 lucide Bot glyph (`M12 8V4H8` geometry) rendered by `frontend/components/chat/RobotAvatar.tsx`, not the previous `/agent-robot.svg` background marker. The `AI回复` card title, `进度日志` title, and live tool-call summary rows stay text-led and must not render extra title-side glyphs such as inline robot, document, or tool icons.
...
- Successful AI-side conversation order is user message -> execution-log card -> final-answer card -> run-scoped artifact cards when logs and a final answer exist. While a run is still generating and no persisted assistant message exists, bounded `assistant_answer_delta` events may render a temporary streaming `AI回复` draft immediately after the execution-log card; the final persisted assistant message replaces that draft once task state refreshes. `assistant_answer_delta` events are answer-content stream chunks and must not be projected as repeated progress-log status rows; the run log should keep a single answer-generation status plus terminal answer-complete status. The execution-log card projects search events, `deep_agent_activity`, `reasoning_trace`, `file_tool_audit`, and warning/error events through safe live summaries for that run; it is not a persisted chat message and must not be fed back as assistant conversation history.
- User message cards expose a copy action after the timestamp whose clipboard text is exactly `message.content`, without timestamps, labels, metadata, or logs. The user-message copy control is visually hidden until hover, keyboard focus, or its copied feedback state so the default user footer remains quiet. Assistant copy support remains on the final-answer card and temporary streamed-answer draft only. Execution-log copy/export uses the full safe run log set, including rows hidden by compact default rendering.
- User-message, final-answer, and execution-log copy controls use a compact two-overlapping-rounded-squares copy glyph in the idle state, inheriting the current button color instead of introducing a separate icon palette. They show a short checkmark feedback state only after a successful clipboard write. The checkmark should stay centered inside the icon button across compact user-message controls and larger assistant/log controls, and remain within the warm cream/coral/dark visual-token system instead of using a separate green success block. Copy failures stay as assistant-style workspace notices and must not fake a success state.
- Frontend final-answer wording should distinguish assistant final content from execution logs with an `AI回复` card title. It must not create persisted extra assistant messages, render the title-side robot glyph, or move tool/log summaries into the final-answer card.
...
- External tool credentials must cross Harness boundaries as `CredentialRef` records, not token strings. Public credential-ref payloads may expose only stable reference id, label, and fingerprint; they must not include provider key values, Authorization headers, `Bearer` values, `api_key`, `access_token`, or `refresh_token` shaped fields.
...
Output: events include search_tool_call, search_tool_result, answer-generation live status, optional assistant_answer_delta stream events, search_synthesis_completed, and search_completed for the run; the assistant message is a synthesized Chinese answer without a backend-appended reference block, while bounded source metadata remains in completion events.
```

Stream a simple answer:

```text
Input: model streaming returns "你好，" then "我正在生成回答。".
Output: the backend appends cumulative assistant_answer_delta events such as "你好，" then "你好，我正在生成回答。"; the frontend renders a temporary generating AI回复 card after the log card until the persisted assistant message arrives.
```

Preserve safe source links:

```text
Input: Tavily returns a source URL such as http://www.weather.com.cn/weather15d/101121701.shtml.
Output: search source metadata, synthesis prompts, and any model-authored Markdown link keep the full external URL; unsafe local or credential-bearing URLs are omitted instead of shown as clickable links.
```
...
- `ExecutionGateway` is currently a compatibility boundary, not a production runtime switch. Legacy gateway wrappers may register existing search, deterministic document analysis, and DeepAgent capabilities for tests and future HarnessEngine extraction, but `TaskRunner` must keep its current call graph until an explicit migration PR; gateway resource references must use virtual `myagent://` URIs rather than local absolute paths.
- `ResourceRef` / `ArtifactRef` are compatibility references until ResourceResolver is explicitly introduced. Upload events and run manifests may carry virtual `myagent://` refs and digests alongside old `filename`, `bytes`, `artifact_names`, and HTTP artifact URLs; refs must not expose local absolute paths or replace existing artifact download endpoints.
- `ContextManager` / `ContextView` is currently a read-only compatibility skeleton. It may build safe message, tool, event-ref, and resource-manifest views from Session events for tests and future HarnessEngine extraction, but it must not change TaskRunner prompts, upload selection, DeepAgent workspaces, API responses, or frontend rendering until an explicit migration PR.
- `SecretVault` / `CredentialRef` is currently a compatibility boundary. It may define how future ToolProxy or external executors request credentials by reference, but it must not migrate real provider keys, expose `.env` paths, change environment-variable loading, or route production tools through a new vault until an explicit credential-management decision.
...
- Frontend normalizes `assistant_answer_delta` into `ExecutionLog.answerStream` only when `schema_version: 1` and bounded cumulative `content` are present. The conversation stream uses the latest valid stream event per run for a temporary `AI回复` draft, and ignores malformed stream payloads instead of exposing arbitrary event data.
...
- Frontend run cards default to user-facing live summaries rather than historical technical reasoning dumps. Running tasks show one active animated status row such as `正在生成回答...` and may show the streamed answer draft beside the run log when answer content has started. Completed history keeps terminal summaries and tool cards without exposing raw reasoning traces by default; streamed answer chunks remain excluded from the log rows and clipboard text to avoid repeated `正在生成回答...` entries. Non-live info/success events should not fall back to raw technical titles in the live log; warning/error fallbacks use generic user-facing status text.
...
- Deterministic document-analysis `tool_result` events must summarize tool outputs by safe shape only, such as result type, count, character length, object keys, or item keys. They must not persist string previews, uploaded document snippets, raw search snippets from document files, or raw tool result bodies in `events.jsonl`, task API payloads, frontend state, DOM, or copied logs.
- Search URL sanitization must distinguish external web URLs from local filesystem paths. A path-redaction rule must not treat the `p:` in `http://` as a Windows drive path, and a URL safety helper must not expose localhost, local IP, percent-encoded hosts, non-web schemes, userinfo, or sensitive query keys as source links.
...
- Security boundary tests should scan `events.jsonl`, `state.json` assistant messages, run artifacts, `records/`, `outputs/`, and agent workspaces for forbidden credential shapes including `DEEPSEEK_API_KEY`, `TAVILY_API_KEY`, `Authorization`, `Bearer `, `sk-`, `api_key`, `access_token`, and `refresh_token`.
...
- Do not persist raw DeepAgents stream chunks, raw LangGraph event payloads, raw provider messages, raw tool results, or raw hidden reasoning in `events.jsonl`, task messages, frontend state, DOM, or clipboard text. The only answer-content streaming exception is the MyAgent-authored `assistant_answer_delta` contract, which stores bounded cumulative assistant-facing text after sanitizer/redaction and must never carry provider reasoning chunks or arbitrary provider envelopes.
...
- Do not repair broken external links in frontend Markdown by guessing. If a source link renders as a local relative URL, fix the backend source URL or sanitizer boundary that produced the damaged href.
...
- `backend/tests/workflow/test_workflow.py`: API-level successful/failing DeepAgent runs, profile selection consistency between run manifest/orchestration event/runtime factory, upload-reference bid prompt routing, search synthesis event order, simple-chat `assistant_answer_delta` stream event order, exactly one user and one assistant message per run, `deep_agent_activity` plus `file_tool_audit` plus `reasoning_trace`, every created run's terminal safe reasoning coverage, generic reasoning idempotency on DeepAgent/document routes, incremental `/events` polling, orchestration decision records, bounded search source summaries, missing-provider fallback summaries, no raw Tavily JSON or provider content in final assistant content/completion event payloads, canonical bid artifacts, evidence pair coverage, and final task status.
...
- Assistant sender row markers drifting from the reference TenderWord blue lucide Bot avatar, or removed title-side glyphs being reintroduced beside `AI回复`, `进度日志`, or live tool summaries after CSS/component rewrites.
