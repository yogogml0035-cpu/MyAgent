...
把上传、资源、产物三段连起来，当前项目的真实路径大致是：

```text
ChatComposer 选择文件
-> partitionSupportedUploadFiles() 做前端体验层筛选
-> POST /api/tasks/{id}/files
-> storage.save_uploads() 做后端权威校验和落盘
-> Runner 注入 resource manifest
-> Agent 通过 read_resource_text / read_resource_table 按需读取
-> 运行结果写成 artifact
-> 前端通过 artifact API 下载或 sandbox 预览
```

这里最值得记住的不是具体函数名，而是三层边界：

- 前端筛选只是体验优化，不是安全边界
- 后端上传校验才是安全边界
- Agent 真正读文件时，仍然必须走 task-scoped resource tools
...
```bash
python Study/chapters/05_uploads_tools_artifacts/mini_unit.py
```
