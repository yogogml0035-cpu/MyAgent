...
如果你是第一次接触这个项目，最稳的节奏通常是：

1. 先读本章 `README.md`，只抓本章主线，不急着背细节。
2. 跑 `mini_unit.py` 或 `mini_unit.mjs`，先看“最小验证”到底在证什么。
3. 故意改坏 README 里提示的那一处，再运行一次，观察为什么失败。
4. 对照 `answers.md`，用自己的话复述“失败意味着真实项目里哪条边界会被破坏”。
5. 最后再回到真实源码，把本章列出的关键文件至少打开一遍。

如果一章读完后，你还不能回答“这章到底在保护哪条系统边界”，说明还不算真正学会。

## 练习失败时怎么判断

- 如果你是按 README 提示故意改坏代码，断言失败通常是正确结果，重点是理解失败在保护什么。
- 如果你没有改坏代码，但源码锚点检查失败，更可能是项目代码移动了或学习资料过时了，这时应该优先用 `rg` 追真实入口。
- 如果是运行命令本身失败，先看本机终端环境是否和文档示例一致；学习资料只保证“概念和源码锚点”正确，不负责部署环境。
...
```text
可学习：README 解释为什么这样设计
-> 可验证：mini_unit 能最小复现关键边界
-> 可自测：answers + final_self_check 让你知道自己哪里没真正看懂
```

如果你已经能跑完 `Study/run_all_mini_units.py`，但 `final_self_check.md` 里很多题还答不出来，说明你完成了“验证”，还没完成“理解”。
